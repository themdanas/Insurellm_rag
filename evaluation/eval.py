...
Provide detailed feedback and scores from 1 (very poor) to 5 (ideal) for each dimension. If the answer is wrong, then the accuracy score must be 1.""",
    ...
    # Access response content (using dictionary access to avoid type checker issues)
    content = judge_response.choices[0].message.content  # type: ignore[attr-defined]
    if content is None:
        raise ValueError("Judge response content is None - LLM did not return any content")
    
    answer_eval = AnswerEval.model_validate_json(str(content))
